...
(function createSlice() {
    if (argv.length === 2)
        throw new Error(
            'Layer and Slice are missing. Usage: node ./scripts/createSlice [layer] [slice]',
        );

    if (argv.length < 4)
        throw new Error(
            'Either Layer or Slice is missing. Usage: node ./scripts/createSlice [layer] [slice]',
        );

    const layer = argv[2].toLowerCase();
    if (!layers.includes(layer))
        throw new Error(
            `Incorrect layer. Available layers: [${layers.toString()}]`,
        );
    ...
    const pathsData = new Map([
        ...
        [uiComponentPath],
        [
            join(uiComponentPath, `${capitalisedSlice}.tsx`),
            componentContent(slice),
        ],
        [
            join(uiComponentPath, `${capitalisedSlice}.stories.tsx`),
            storiesContent(layer, slice),
        ],
        [
            join(uiComponentPath, `${capitalisedSlice}.module.scss`),
            styleContent(slice),
        ],
        [modelPath],
        [selectorsPath],
